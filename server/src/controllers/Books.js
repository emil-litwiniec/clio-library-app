...
const Books = {
  async insert(req, res) {
    if (
      ...
      return res
        .status(400)
        .send({ message: 'Please, provide all required values...' });
    }
  ...
  async remove(req, res) {
    if (!req.body.bookId) {
      return res
        .status(400)
        .send({ message: 'Provide id of the book to delete' });
    }
  ...
  async update(req, res) {
    if (!req.body.bookId) {
      return res
        .status(200)
        .send({ message: 'Provide id of the book to update' });
    }
      ...
      try {
        ...
        if (!authorId[0]) {
          ...
          if (!authorResponse.rows[0]) {
            return res.status(400).send({
              message:
                'Something went wrong with adding author to the database.'
            });
          }
        }
        const filteredRequestEntries = Object.entries(req.body).filter(
          entry => {
            if (
              entry[0] === 'bookId' ||
              entry[0] === 'authorFirst' ||
  ...
  async getBook(req, res) {
    if (!req.body.bookId) {
      return res
        .status(400)
        .send({ message: 'Please, provide id of the book.' });
    }
    const getBookQuery =
